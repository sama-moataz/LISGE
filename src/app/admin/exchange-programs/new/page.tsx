...
import { Loader2, Save, ShieldAlert, ArrowLeft, UploadCloud, Globe2 } from 'lucide-react';
import type { ExchangeProgram, LocationFilter, AgeFilter, FundingFilter, RegionFilter, LevelFilter, FundingCountryFilter } from '@/types'; // Updated types
import { useToast } from "@/hooks/use-toast";
import Link from 'next/link';
import Image from 'next/image';
import { handleAddExchangeProgramAction } from '../actions';
...
const locationOptions: { value: ExchangeProgram['location']; label: string }[] = [
...
const exchangeProgramSchema = z.object({
...
type ExchangeProgramFormData = z.infer<typeof exchangeProgramSchema>;

export default function NewExchangeProgramPage() {
  ...
  const { control, handleSubmit, formState: { errors }, watch, setValue } = useForm<ExchangeProgramFormData>({
    resolver: zodResolver(exchangeProgramSchema),
    defaultValues: {
      name: '', description: '', eligibility: '', websiteUrl: '',
      iconName: null, category: null, location: 'International', ageRequirement: null,
      fundingLevel: null, destinationRegion: null, targetLevel: null, fundingCountry: null,
      partner: null, coverage: null, deadline: null, duration: null, imageUrl: null,
    },
  });
  ...
  useEffect(() => {
    if (!authLoading) {
      if (!user) {
        router.replace('/auth/login?redirect=/admin/exchange-programs/new');
      } else if (!isAdmin) {
        toast({ title: "Access Denied", description: "You do not have permission to add exchange programs.", variant: "destructive" });
        router.replace('/dashboard');
      }
    }
  }, [user, isAdmin, authLoading, router, toast]);

  const onSubmit: SubmitHandler<ExchangeProgramFormData> = async (data) => {
    setIsSubmitting(true);
    
    const processedDataForAction: Omit<ExchangeProgram, 'id' | 'createdAt' | 'updatedAt' | 'icon' | 'dataAiHint'> = {
        ...data,
    ...
    const result = await handleAddExchangeProgramAction(processedDataForAction);

    if (result.success) {
      toast({ title: "Success", description: `Exchange Program "${result.name}" added successfully.` });
      router.push('/admin/exchange-programs');
    } else {
      toast({ title: "Error Adding Program", description: result.error || "Failed to add exchange program.", variant: "destructive" });
    }
    ...
    <div className="space-y-6 max-w-3xl mx-auto">
      <div className="flex items-center justify-between">
        <Button variant="outline" size="sm" asChild>
          <Link href="/admin/exchange-programs">
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-2xl font-headline">Add New Exchange Program</CardTitle>
          <CardDescription>Fill in the details for the new exchange program.</CardDescription>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            <div>
              ...
              <Controller
                name="name"
                control={control}
                render={({ field }) => <Input id="name" {...field} placeholder="E.g., Global Scholars Exchange" />}
              />
            ...
            <div>
              ...
              <Controller
                name="description"
                control={control}
                render={({ field }) => <Textarea id="description" {...field} rows={3} placeholder="Detailed description..." />}
              />
            ...
            <div>
              ...
              <Controller
                name="eligibility"
                control={control}
                render={({ field }) => <Textarea id="eligibility" {...field} rows={2} placeholder="E.g., University students, specific major" />}
              />
            ...
            <div>
              ...
              <Controller
                name="websiteUrl"
                control={control}
                render={({ field }) => <Input id="websiteUrl" {...field} placeholder="https://example.com/exchange" />}
              />
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                ...
                <Controller
                  name="category"
                  control={control}
                  render={({ field }) => <Input id="category" {...field} value={field.value ?? ''} placeholder="E.g., Academic, Cultural" />}
                />
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    <Label htmlFor="location">Program Location (Operator Base) <span className="text-destructive">*</span></Label>
                    <Controller
                    name="location"
                    control={control}
                    render={({ field }) => (
            ...
            <div>
              <Label htmlFor="partner">Main Partner/Sponsor</Label>
              <Controller
                name="partner"
                control={control}
                render={({ field }) => <Input id="partner" {...field} value={field.value ?? ''} placeholder="E.g., University, Government Body, NGO" />}
              />
            ...
            <div>
              ...
              <Controller
                name="coverage"
                control={control}
                render={({ field }) => <Textarea id="coverage" {...field} value={field.value ?? ''} rows={2} placeholder="E.g., Tuition, travel, accommodation, stipend" />}
              />
             ...
             <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  <Controller
                    name="duration"
                    control={control}
                    render={({ field }) => <Input id="duration" {...field} value={field.value ?? ''} placeholder="E.g., 1 Semester, Full Academic Year, 3 Weeks" />}
                  />
